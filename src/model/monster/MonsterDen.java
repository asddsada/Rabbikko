...
public class MonsterDen {
			...
			public void run() {
				while (true) {
					...
					if (Main.isGameRunning
							&& RenderableHolder.getInstance().size() < Constant.MAX_NUMBER_RENDERABLE_HOLD
							&& GameLogic.hero.isAlive()) {
	...
	public void genMonster(int img, int row, int col, int size, int time) {
		...
		for (int i = 0; i < time; i++) {
			Dungeon.addEntities(new Monster(monsterImg(img), row, col, speed, mass, hp * ((dunLvl / 20) + 1), Math.max(
					(int) ((((2 - img % 3) / 3.0 * 300.0) + 13 * (col * img) + 6 * (row) + hp / 1377.0 * 400) / 2), 15),
					atr, idle, (1 + img % 3), (3 - img % 3) * 300, speed * (3 - img % 3) * 40,
